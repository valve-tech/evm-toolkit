...
Signs a full EIP-4361 SIWE message generated by `createSiweMessage` from `viem/siwe`, with a one-time server nonce issued by `nonceStore.issue()` (from `@valve-tech/siwe-store`). On verify, the server calls `parseSiweMessage`, consumes the nonce via `nonceStore.consume` (replay protection), validates domain + expiry via `validateSiweMessage`, and recovers the signer address via `recoverMessageAddress`. If it matches the claimed address, an opaque random session token is issued. The signature proves *who you are* to the server. It is consumed immediately; the session token is the only thing that persists.
