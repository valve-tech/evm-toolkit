...
| Capability | How |
|---|---|
| Wallet login (full EIP-4361 SIWE) | server `createSiweMessage` (nonce from `siwe-store`) → client `signMessage` → server `parseSiweMessage` + `nonceStore.consume` + pin `version`/`uri`/`chainId` + `validateSiweMessage` + verify signature (EOA + EIP-1271/6492) → opaque session token |
...
connectWallet() + getChainId()
  ...
  └─ POST /auth/verify         ──►  parseSiweMessage + nonceStore.consume +
       { message, signature }       pin version/uri/chainId +
                              ◄──  validateSiweMessage + verify sig (EOA+1271/6492)
                                    { token, address }
...
**Prompt 1 — auth challenge (EIP-4361 SIWE)**

Signs a full EIP-4361 SIWE message generated by `createSiweMessage` from `viem/siwe`, with a one-time server nonce issued by `nonceStore.issue()` (from `@valve-tech/siwe-store`). On verify (`server/siwe-auth.ts`), the server: consumes the nonce via `nonceStore.consume` (replay protection); re-asserts the binding fields `validateSiweMessage` does **not** check — `version`/`uri`/`chainId` — against server config (EIP-4361 defense-in-depth); validates domain + expiry via `validateSiweMessage`; then verifies the signature. Verification is a hybrid: an offline ECDSA recover for EOAs, falling back to a viem `PublicClient.verifyMessage` for **EIP-1271 / EIP-6492 smart-contract accounts** (Safe, AA wallets, counterfactual deployments). Only the smart-account path uses an RPC (`SIWE_RPC_URL`, pointed at the `SIWE_CHAIN_ID` chain); EOA logins never touch it. On success an opaque random session token is issued. The signature proves *who you are* to the server. It is consumed immediately; the session token is the only thing that persists.
